...
COPY --from=build /usr/src/app/ruoyi-admin/target/*.jar 
COPY --from=build /usr/src/app/ruoyi-common/target/*.jar 
COPY --from=build /usr/src/app/ruoyi-framework/target/*.jar 
COPY --from=build /usr/src/app/ruoyi-generator/target/*.jar 
COPY --from=build /usr/src/app/ruoyi-quartz/target/*.jar 
